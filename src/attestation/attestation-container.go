...
import (
	...
	pb "github.com/azure/azure-cleanroom/src/attestation/protobuf"
...
func main() {
	...
	// Create a channel to listen for termination signals and start the gRPC server in a go routine
	// which can then be stopped gracefully.
	...
	// Wait for SIGINT/SIGTERM. When running in a container this lets the container exit
	// gracefully with exit code 0.
	sig := <-sigs
	logrus.Infof("Received signal: %v. Shutting down gRPC server...", sig)
	s.GracefulStop()
